      ...
      if (!MCH_Config.HideKeybind.prmBool) {
         ...
         if (info != null) {
            ...
            if (seatID == 0 && info.isEnableGunnerMode && !Keyboard.isKeyDown(MCH_Config.KeyFreeLook.prmInt)) {
               stat = plane.isHoveringMode() ? colorInactive : colorActive;
               msg = (plane.getIsGunnerMode(player) ? "Normal" : "Gunner") + " : " + MCH_KeyName.getDescOrName(MCH_Config.KeySwitchMode.prmInt);
               this.drawString(msg, RX, this.centerY - 70, stat);
            }

            String msg;
            if (seatID > 0 && plane.canSwitchGunnerModeOtherSeat(player)) {
               msg = (plane.getIsGunnerMode(player) ? "Normal" : "Camera") + " : " + MCH_KeyName.getDescOrName(MCH_Config.KeySwitchMode.prmInt);
            ...
            if (plane.canEjectSeat(player)) {
               msg = "Eject seat: " + MCH_KeyName.getDescOrName(MCH_Config.KeySwitchHovering.prmInt);
            ...
            if (plane.getIsGunnerMode(player) && info.cameraZoom > 1) {
               ...
               if (!plane.canFoldWing() && !plane.canUnfoldWing()) {
                  ...
                  msg = "FoldWing : " + MCH_KeyName.getDescOrName(MCH_Config.KeyZoom.prmInt);
                  this.drawString(msg, LX, this.centerY - 80, colorActive);
               }
            }
